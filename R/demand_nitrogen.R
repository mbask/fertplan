...
demand_nitrogen <- function(soil_dt, blnc_cmpt) `: dt` ({
  ...
  demand_dt <- soil_dt[
    , `:=` (
      b1_N_kg_ha             = b1_available_n(
        total_n_pc     = N_pc,
        texture        = texture),
      b2_N_kg_ha             = b2_mineralized_n(
        crop_type      = crop_type,
        som_pc         = SOM_pc,
        cn_ratio       = CNR,
        texture        = texture))]
  demand_dt[
    , `:=` (
      A_N_kg_ha              = A_crop_demand(
        crop_abs       = rem_n_coef_of(crop, part) / 100,
        crop_exp_yield = expected_yield_kg_ha),
      B_N_kg_ha              = B_N_in_soil(b1_N_kg_ha, b2_N_kg_ha),
      C_N_kg_ha              = C_N_precip_leach(
        available_n      = b1_N_kg_ha,
        rainfall_oct_jan = oct_jan_pr_mm))]
  demand_dt[
    , `:=` (
      D_N_kg_ha              = D_N_denitrification(
        B             = B_N_kg_ha,
        drainage_rate = drainage_rate,
        soil_texture  = texture),
      E_N_kg_ha              = E_N_from_prev_crop(crop = prev_crop),
      F_N_kg_ha              = F_N_prev_fertilization(n_supply_prev_frt_kg_ha, organic_fertilizer, years_ago),
      G_N_kg_ha              = G_N_from_atmosphere(coeff = n_supply_atm_coeff))]
